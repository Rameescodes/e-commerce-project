...
const razorpay = new Razorpay({
    key_id: "rzp_test_lalA2gdDUC1WmY",

    key_secret: "kfR0Cyu5KzPqUJtZ7skdBVCB",
});
...
const calculateSubtotal = (cart) => {
    ...
    for (const cartItem of cart) {
        ...
        const priceToConsider = cartItem.product.price;
...
const checkoutPage = async (req, res) => {
    try {
        ...
        const orders = await cart.findOne({ user: userId }).populate('item.product')

        const addresses = await address.find({ user: userId }).sort({ createdDate: -1 }).exec();

        const productTotal = calculateSubtotal(orders.item);

        res.render('checkoutpage', { orders: orders.item, productTotal, addresses })
    } catch (error) {
...
const razorpayOrder = async (req, res) => {
    try {
        ...
        for (const cartItem of cartItems) {
            ...
            const priceToConsider = product.price;
            product.quantity -= cartItem.quantity;
